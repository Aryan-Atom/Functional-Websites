...
const Navbar = () => {
  const links = [
    { link: "Home", isActive: true },
    { link: "Work", isActive: false },
    { link: "Culture", isActive: false },
    { link: "", isActive: false },
    { link: "News", isActive: false },
  ];
  return (
    <div className="max-w-screen-xl mx-auto py-6 flex items-center gap-3 justify-between border-b border-zinc-700">
      <div className="nleft flex">
        ...
        <div className="nav-links flex gap-10 ml-20">
          {links.map((link, index) => (
            <a key={index} className="text-sm flex items-center gap-1" href="#">
              {link.isActive && (
                <span
                  style={{ boxShadow: "0 0 0.45em #00FF19" }}
                  className="inline-block w-1.5 h-1.5 bg-green-300 rounded-full"
                ></span>
              )}
              {index === 3 ? (
                <span className="h-5 w-0.5 bg-zinc-600"></span>
              ) : (
                link.link
              )}
            </a>
